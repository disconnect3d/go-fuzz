...
import (
	...
	. "github.com/disconnect3d/go-fuzz/internal/go-fuzz-types"
)
...
func (c *Context) loadPkg(pkg string) {
	...
	loadpkgs := []string{pkg, "github.com/disconnect3d/go-fuzz/go-fuzz-dep"}
	if *flagLibFuzzer {
...
func (c *Context) calcIgnore() {
	...
	roots := c.packagesNamed("runtime", "github.com/disconnect3d/go-fuzz/go-fuzz-dep")
...
func (c *Context) copyFuzzDep() {
	...
	c.mkdirAll(newDir)
	dep := c.packageNamed("github.com/disconnect3d/go-fuzz/go-fuzz-dep")
	for _, f := range dep.GoFiles {
		data := c.readFile(f)
		// Eliminate the dot import.
		data = bytes.Replace(data, []byte(`. "github.com/disconnect3d/go-fuzz/go-fuzz-defs"`), nil, -1)
		c.writeFile(filepath.Join(newDir, filepath.Base(f)), data)
	}

	defs := c.packageNamed("github.com/disconnect3d/go-fuzz/go-fuzz-defs")
